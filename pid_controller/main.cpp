...
int closest_point_index_between_trajectory_planner_and_vehicle_location(vector<double>trajectory_x_points, vector<double>trajectory_y_points, double vehicle_x,double vehicle_y )
{
  double closest_distance = std::numeric_limits<double>::infinity();
  int closest_point_index;
  double dist;
  for (int i = 0;i<trajectory_x_points.size();i++)
  {
    // look for closest point in forward direction when driving forward
    // if (trajectory_y_points[i] < vehicle_y)
    // {
      // std::cout<< trajectory_y_points[i] - vehicle_y << "--------------- distance ---------------"  << std::endl;
      dist = std::sqrt(std::pow((trajectory_x_points[i] - vehicle_x),2) + std::pow((trajectory_y_points[i] - vehicle_y),2));
  ...
  std::cout<< closest_distance << " -------- closest distance ------" << trajectory_x_points.size()<<" trajectory soze "<< std::endl;
  return closest_point_index;
}

// return (x_avg, y_avg) from path planner generated waypoints, this will be used to formulate error 
std::vector<double> avg_waypoints(vector<double>trajectory_x_points, vector<double>trajectory_y_points)
{
  std::vector<double> avg_trajectory_points;
  double avg_x, avg_y;
  for (auto each:trajectory_x_points)
  {
    avg_x += each;
  }
  for(auto each:trajectory_y_points)
  {
    avg_y +=each;
  }
  avg_trajectory_points.push_back(avg_x/(trajectory_x_points.size()-1));
  avg_trajectory_points.push_back(avg_y/(trajectory_y_points.size()-1));
}
...
int main ()
{
  ...
  **/
  ...
  // pid_steer.Init(0.3,0.001,0.1,1.2,-1.2); //bad
  // pid_steer.Init(0.3,0.001,0.03,1.2,-1.2); //bad
  // pid_steer.Init(0.3,0.01,0.3,1.2,-1.2);// OK
  // pid_steer.Init(0.3,0.02,0.3,1.2,-1.2); // OK
  // pid_steer.Init(0.3,0.03,0.3,1.2,-1.2); //OK
  // pid_steer.Init(0.3,0.04,0.3,1.2,-1.2); //OK
  ...
  // pid_throttle.Init(0.32,0.01,0.2,1,-1);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          // compute angle difference b/t goal yaw and current headings
          int closest_index =  closest_point_index_between_trajectory_planner_and_vehicle_location(x_points, y_points, x_position, y_position);
          // error_steer = (goal.rotation.yaw/180*M_PI - angle_between_points( x_points[closest_index], x_points[closest_index],x_position, y_position));
          // *  method 1 compute error by difference b/t first points in waypoints x,y 
          error_steer = 3*(y_position - y_points[10])  + 0.08*(x_position - x_points[10]);


          // // *  method 2 compute error by angle b/t ( avg_x, avg_y, ego_x, ego_y ) and ego_yaw
          // double x_avg = std::accumulate(x_points.begin(), x_points.end(),0.0)/x_points.size();
          // double y_avg = std::accumulate(y_points.begin(), y_points.end(),0.0)/y_points.size();
          // double planed_yaw = angle_between_points(x_avg,y_avg,x_position,y_position);
          // error_steer = planed_yaw - check_angle(yaw);
          // error_steer = check_angle(error_steer);
...
//           // Compute control to apply
          ...
          std::cout<< "debug \n" <<  std::endl;
          std::cout<< " num of waypoints in x_points    " << x_points.size() << std::endl;
          std::cout<< " current yaw " << yaw << std::endl;
          std::cout<< " yaw error " << error_steer  << " next yaw " << steer_output << "\n"<< std::endl;
          std::cout<< "ego x_position " << x_position << "ego y_position " << y_position << std::endl;
          std::cout<< "waypoint x " << waypoint_x <<" waypoint y " <<waypoint_y << std::endl;
          std::cout<< "ego yaw " << yaw << std::endl;
          std::cout<< "waypoint yaw " << waypoint_t/180*M_PI << std::endl;
...
//           // Save data
          ...
          **/
          ...
          double brake_output;
          double throttle;
          // *  method 1 use last waypoint speed in throttle error
          if (v_points.size() > 14)
          {
            error_throttle = 0.9*(velocity - v_points.back());
            // Compute control to apply
          ...
          {
            throttle = 0;
            std::cout<< "------------------------ too less waypoints, do not move -------------------" << std::endl;
          }
          
          // * method 2 use avg waypoints speed to calculate error 
          // double v_avg = std::accumulate(v_points.begin(),v_points.end(), 0.0)/v_points.size();
